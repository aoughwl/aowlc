// aowlc — a .c.nif -> C native backend for nimony.
//
...
// Constructs it has not lowered yet raise `aowlc: unsupported …` so gaps are
...
class Emitter {
  ...
  genType(t) {
    ...
    if (!isList(t)) throw new Error("aowlc: bad type node");
    switch (t.tag) {
      ...
      default:
        ...
        throw new Error("aowlc: unsupported type '" + t.tag + "'");
    }
  }
  genProcTypePtr(t) {
  ...
  genExpr(e) {
    ...
    if (!isList(e)) throw new Error("aowlc: bad expr node");
    const t = e.tag;
    if (TYPED_BIN[t]) {
  ...
  genKeepOverflow(s) {
    ...
    if (!builtin) throw new Error("aowlc: unsupported keepovf op '" + op.tag + "'");
...
// ----------------------------------------------------------------------------
const PRELUDE = `/* GENERATED BY aowlc. DO NOT EDIT. */
...
function compileModule(snif, opts = {}) {
  ...
  if (inits.length) {
    out += "static void __attribute__((constructor)) aowlc_init(void) {\n" + inits.join("\n") + "\n}\n";
  }
  return indentC(out);
}
...
function compileHarness(snif, entry, argExprs = []) {
  ...
  if (!entryName) throw new Error("aowlc: entry proc '" + entry + "' not found. Available: " +
